...
public:
	...
	struct cbObject
	{
		...
		// これは"HLSL"側で"bool"型があるが16バイトアライメントを守るのに適していないから"ing"型にしている
		int             IsSkinMeshObj = 0;
		...
		// アルファディザー関連
		int   AlphaDitherEnable         = 0;
		float AlphaDitherEnableDistance = 0.0f;
	...
	// トゥーンテクスチャ設定
	void SetToonTexture(KdTexture& toonMask)
	{
		KdDirect3D::Instance().WorkDevContext()->PSSetShaderResources(13 , 1 , toonMask.WorkSRViewAddress());
	}
	...
	// 守内追加
	// アルファディザー
	void EnableAlphaDither (bool IsEnable , float EnableDistance);
	void DisableAlphaDither();
