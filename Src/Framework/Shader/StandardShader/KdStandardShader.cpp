...
void KdStandardShader::EnableAlphaDither(bool IsEnable, float EnableDistance)
{
	m_cb0_Obj.Work().AlphaDitherEnable         = IsEnable;
	m_cb0_Obj.Work().AlphaDitherEnableDistance = EnableDistance;

	m_dirtyCBObj = true;
}

void KdStandardShader::DisableAlphaDither()
{
	m_cb0_Obj.Work().AlphaDitherEnable         = false;
	m_cb0_Obj.Work().AlphaDitherEnableDistance = 0.0f;
...
// ///// ///// ///// ///// ///// ///// ///// ///// ///// ///// ///// ///// ///// ///// /////
// モデルワークを描画（ダイナミック(アニメーションをする)モデルに対応
// ===== ===== ===== ===== ===== ===== ===== ===== ===== ===== ===== =====
	...
	const Math::Color& colRate, const Math::Vector3& emissive)
{
	...
	// スキンメッシュモデルの場合 : ボーン情報鵜を書き込み(スキンメッシュ対応)
	if (data->IsSkinMesh())
	{
		// ノード内からボーン情報を取得
		for(auto&& nodeIdx : data->GetBoneNodeIndices())
		{
			if (nodeIdx >= KdStandardShader::maxBoneBufferSize) { assert(0 && "転送できるボーンの上限を超えました"); return; }

			auto& dataNode = dataNodes[nodeIdx];
			auto& workNode = workNodes[nodeIdx];

			// ボーン情報から"GPU"に渡す行列の計算
...
#include "KdStandardShader_PS_UnLit.shaderInc"
	...
	// 守内追加
	SetToonTexture(*KdAssets::Instance().m_textures.GetData("Asset/Texture/System/Toon.png"));
