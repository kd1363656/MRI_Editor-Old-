...
float4 main(VSOutput In) : SV_Target0
{
	...
	// アルファディザ
	if (g_AlphaDitherEnable == 1 && camDist <= g_AlphaDitherEnableDistance)
	{
		// ベイヤー行列の取れる値が最大"15"だから
		const float thresholdScale = 16.0f;

		// ピクセルのスクリーン座標からインデックスを取得(スクリーン座標から導出)
		int x_ = (int)fmod(In.Pos.x , 4.0f);
		int y_ = (int)fmod(In.Pos.y , 4.0f);

		float threshold_ = BayerMatrix[y_][x_];

		// カメラの距離出ディザーの強さを変えている
		float ditherFactor_ = saturate(camDist / g_AlphaDitherEnableDistance);

		float visibility_ = smoothstep(threshold_ - 8.0f , threshold_ + 0.8f , baseColor.a * ditherFactor_ * 16.0f);
		baseColor.a *= visibility_;
		
		// "Discard"はピクセルの描画を完全中止する命令
		// フェードのアルファがほぼ消えるレベルなら描画しないため"Discard"
