...
public:
	...
	// 回転対応描画関数
	void DrawTex(const std::weak_ptr<KdTexture> tex , const Math::Rectangle* srcRect = nullptr , const Math::Color* color = &kWhiteColor , const Math::Vector2& pivot = { 0.5f , 0.5f })
	{
		if (tex.expired()) return;
		DrawTex(tex.lock().get(), 0, 0, srcRect->width , srcRect->height, srcRect, color, pivot);
	}
