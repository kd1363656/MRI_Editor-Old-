...
void KdShaderManager::WriteCBCamera(const Math::Matrix& cam, const Math::Matrix& proj)
{
	...
	Math::Vector2 window_ = { static_cast<float>(app.GetWindowSize().width) , static_cast<float>(app.GetWindowSize().height) };

	camera.screenSize    = { window_.x , window_.y};
	// 一ピクセルに占める解像度による倍率
	camera.screenInvSize = { 1.0f / window_.x , 1.0f / window_.y};
