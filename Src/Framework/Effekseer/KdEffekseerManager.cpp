...
std::weak_ptr<KdEffekseerObject> KdEffekseerManager::Play(const std::string& effName, const DirectX::SimpleMath::Vector3& pos, const float size, const float speed, const bool isLoop)
{
	PlayEfkInfo info;
...
{
	float deltaTime = Application::Instance().GetScaledDeltaTime();
