...
public:
	...
	enum Type
	{
		...
		TypeDebug      = 1 << 6,	// デバッグ         : デバックでマウスでクリックした位置にまっすぐにレイを飛ばしてゲームオブジェクトを取得する際に使う
	};
	...
	{
		KdCollider::ShapeType m_shapeType = KdCollider::ShapeType::None;
		...
		UINT m_collisionType = 0u;

		std::weak_ptr<GameObject> m_hitGameObject;
	};
	...
	// 当たり判定実行
	bool Intersects(std::weak_ptr<GameObject> GameObject  , const SphereInfo& targetShape , const Math::Matrix& ownerMatrix , std::list<KdCollider::CollisionResult>* pResults) const;
	...
	bool Intersects(std::weak_ptr<GameObject> GameObject , const BoxInfo&    targetBox   , const Math::Matrix& ownerMatrix , std::list<KdCollider::CollisionResult>* pResults) const;
	bool Intersects(const BoxInfo&		      targetBox  , const Math::Matrix& ownerMatrix , std::list<KdCollider::CollisionResult>* pResults) const;

	bool Intersects(std::weak_ptr<GameObject> GameObject  , const RayInfo&    targetShape , const Math::Matrix& ownerMatrix , std::list<KdCollider::CollisionResult>* pResults) const;
